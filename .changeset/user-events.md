...
- The listener runs after the write has landed and is awaited before the flow answers, so a durable queue has the event by then. A listener that throws fails no flow: the write happened. The failure is a `JANUS_EVENT_FAILED` warning that names the event type, its id and the user id, never the failure's message.
