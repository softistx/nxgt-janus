...
- **A user's tokens spent in one statement, v0.3.0** — for `@nxgt/janus`
  0.7: `spendUserTokens(userId, kind, at, except?)` spends the unspent
  tokens of one user and kind — but `except` — in one `update`, and passes
  the four new conformance cases. No migration: it reads the `user_id` column `deleteUserTokens`
  already filters on.
